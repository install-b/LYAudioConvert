...
- (OSStatus)doConvert:(CFURLRef )sourceURL outPuturl:(CFURLRef )outPutURL outputFormat:(OSType )outputFormat outputSampleRate:(Float64)slr mChannelsPerFrame:(int)mChannels
{
    ...
    try {
        ...
        OSStatus status_rc = ExtAudioFileOpenURL(sourceURL, &sourceFile);
        if (status_rc) {
            NSAssert(0, @"ExtAudioFileOpenURL failed");
        }
        //XThrowIfError(status_rc, "ExtAudioFileOpenURL failed");
    
        // get the source data format
        UInt32 size = sizeof(srcFormat);
        status_rc = ExtAudioFileGetProperty(sourceFile, kExtAudioFileProperty_FileDataFormat, &size, &srcFormat);
        if (status_rc) {
            NSAssert(0, @"couldn't get source data format");
        }
        ...
        if (outputFormat == kAudioFormatLinearPCM) {
            ...
            status_rc = AudioFormatGetProperty(kAudioFormatProperty_FormatInfo, 0, NULL, &size, &dstFormat);
            if (status_rc) {
                NSAssert(0, @"couldn't create destination data format");
            }
        ...
        status_rc = ExtAudioFileCreateWithURL(outPutURL, kAudioFileCAFType, &dstFormat, NULL, kAudioFileFlags_EraseFile, &destinationFile);
        if (status_rc) {
            NSAssert(0, @"ExtAudioFileCreateWithURL failed!");
        }
        ...
        status_rc = ExtAudioFileSetProperty(sourceFile, kExtAudioFileProperty_ClientDataFormat, size, &clientFormat);
        if (status_rc) {
            NSAssert(0, @"ouldn't set source client format");
        }
        ...
        size = sizeof(clientFormat);
        status_rc = ExtAudioFileSetProperty(destinationFile, kExtAudioFileProperty_ClientDataFormat, size, &clientFormat);
        if (status_rc) {
            NSAssert(0, @"couldn't set destination client format");
        }
        ...
        status_rc = ExtAudioFileGetProperty(destinationFile, kExtAudioFileProperty_AudioConverter, &size, &audioConverter);
        if (status_rc) {
            NSAssert(0, @"Couldn't get Audio Converter!");
        }
        ...
        while (1) {
            ...
            status_rc = ExtAudioFileRead(sourceFile, &numFrames, &fillBufList);
            if (status_rc) {
                NSAssert(0, @"ExtAudioFileRead failed!");
            }
            ...
            if (error) {
                ...
                if (kExtAudioFileError_CodecUnavailableInputConsumed == error) {
                    ...
                    sourceFrameOffset -= numFrames;
                    status_rc = ExtAudioFileSeek(sourceFile, sourceFrameOffset);
                    if (status_rc) {
                        NSAssert(0, @"ExtAudioFileSeek failed!");
                    }
